    ...
    for row in csv_data:
        for key in ['ProcessorNumber', 'CpuName', 'Model', 'Family', 'Line']:
            ...
            if val and normalized_extracted in normalize_model(val):
                print(f"Включение в {key}: {val}")
                return row
    print(f"Совпадение не найдено для: {cpu_name}")
    return None
...
def enrich_cpu_data():
    ...
    for cpu in data['CPU']:
        ...
        if match:
            # Обогащаем данные
            cpu['ProcessorNumber'] = match.get('ProcessorNumber', '')
            cpu['CoreCount'] = match.get('CoreCount', '')
            cpu['ThreadCount'] = match.get('ThreadCount', '')
            cpu['ClockSpeed'] = match.get('ClockSpeed', '')
            cpu['Cache'] = match.get('Cache', '')
            cpu['MaxMem'] = match.get('MaxMem', '')
            cpu['Lithography'] = match.get('Lithography', '')
    ...
    # Показываем необогащенные CPU
    not_enriched = [cpu['name'] for cpu in data['CPU'] if not cpu.get('ProcessorNumber')]
    if not_enriched:
        print("\nНе обогащены:")
