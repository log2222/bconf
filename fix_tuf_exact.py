...
def fix_tuf_by_index():
    """Исправляет видеокарту по индексу 179 (позиция 180)"""
    
    print("🔧 Исправляем видеокарту по индексу 179 (позиция 180)...")
    ...
    gpus = data['GPU']
    target_index = 179  # Позиция 180 (индекс 179)
    
    if target_index >= len(gpus):
        print(f"❌ Индекс {target_index} превышает количество видеокарт ({len(gpus)})")
        return False
    
    target_gpu = gpus[target_index]
    print(f"✅ Найдена видеокарта на позиции {target_index + 1}:")
    print(f"   Название: {target_gpu.get('name', 'Не указано')}")
    print(f"   Текущее clean_name: {target_gpu.get('clean_name', 'Не указано')}")
    
    # Ищем соответствующую видеокарту в базе данных
    matching_gpu = find_matching_gpu(target_gpu['name'], gpu_database)
    
    if matching_gpu:
        print(f"✅ Найдено правильное совпадение: {matching_gpu.get('name', 'Неизвестно')}")
        
        # Принудительно обогащаем данные
        enriched_gpu = enrich_gpu_data(target_gpu, matching_gpu)
