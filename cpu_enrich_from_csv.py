                            ...
                            if article == pid:
                                ...
                                if cpu_series:
                                    ...
                                    if not model or model == 'on' or len(model) < 8:
                                        ...
                                        if 'ATHLON' in model.upper():
                                            row_series = 'ATHLON'
                                        elif 'RYZEN' in model.upper():
                                            row_series = 'RYZEN'
                                        elif 'THREADRIPPER' in model.upper():
                                            row_series = 'THREADRIPPER'
                                    
                                    # Если серия не совпадает, пропускаем
                                    if row_series and cpu_series != row_series:
                                        print(f"Пропускаем: серия не совпадает (ищем {cpu_series}, найдено {row_series})")
                                        continue
                                
                                print(f"Найдено по {key}: {product_ids}")
                                return row
        ...
        if match:
            ...
            if vendor == "amd":
                ...
                if "ATHLON" in name.upper():
                    ...
                    if match.get('Family'):
                        clean_name = match['Family'].replace('(OEM Only)', '').replace('7th Gen ', '').replace(' APU', '').strip()
                        cpu['clean_name'] = clean_name
        else:
